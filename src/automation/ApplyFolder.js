...
const ApplyFolder = () => {
    ...
    const automoveActionsForm = [
        ...
        'Add wiki page'];
    const [folders, setFolders] = useState([{ id: 1 }]);
    const handleAddFolder = (action) => {
        if (action === 'Apply folder template') {
            // Increment the number for the new folder
            const newFolderNumber = folders.length + 1;
            const newFolder = { id: newFolderNumber };
    ...
    const handleDeleteFolder = (id) => {
        ...
        const updatedFoldersWithIds = updatedFolders.map((folder, index) => ({ ...folder, id: index + 1 }));
        setFolders(updatedFoldersWithIds);
    };
    ...
    return (
        <>
            <div className='folder-content col-12' style={{ padding: '20px', display:'flex' , flexDirection:'column'}}>
                <div className='folder-template col-12' style={{ display: 'flex', gap: '10px', marginBottom: '5%', flexDirection: 'column' }}>
                    {folders.map((folder, index) => (
                        <div key={folder.id} className="folder-card" style={{ border: '1px solid #B2BEB5', borderRadius: '10px', padding: '10px' }}>
                            {/* Render folder content */}
                            <div className='folder-header col-12' style={{ display: 'flex', justifyContent: 'space-between', }}>
                                <div className='folder-header-title-first col-10' style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', }}>
                                    <p style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '10px', }}>
                                        <RxDragHandleDots2 />
                                        <h2 style={{ fontSize: '20px' }}>{folder.id}. Apply folder template</h2>
                                    </p>
                                </div>
                                <div className='folder-header-title-second col-4' style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', float: 'right' }}>
                                    <p style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '10px' }}>
                                        <div style={{ color: 'blue', cursor: 'pointer', display: 'flex', gap: '5px' }}>
                                            <IoMdFolderOpen /> <h5>New template</h5>
                                        </div>
                                        <RiDeleteBin6Line style={{ color: 'red', cursor: 'pointer' }} onClick={() => handleDeleteFolder(folder.id)} />
                                    </p>
                                </div>
                            </div>
                            <div className='folder-body' style={{ paddingTop: '50px' }}>
